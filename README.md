...
> 当前为 `0.3.0` 开源预览版。请先在测试 Vault 中使用，并自行备份重要资料。
...
- 使用 DeepSeek 补全标题、作者、年份、会议/期刊、摘要和研究主题。
...
- DeepSeek 根据 PDF 正文、标注和笔记生成十问式论文导读，并支持围绕单篇论文继续对话。
...
2. 解压到 Vault 的 `.obsidian/plugins/research-os/`。
3. 确认目录中包含 `main.js`、`manifest.json` 和 `styles.css`。
4. 重启 Obsidian，在“设置 → 社区插件”中启用 **Research OS**。
...
在 `设置 → Research OS → DeepSeek AI` 中填写自己的 API Key。AI 功能是可选的；未配置时仍可使用本地文献、阅读队列、笔记和主题功能。

分析优先使用摘要、用户笔记、PDF 高亮和相关正文片段，再按需要补充 PDF 正文，以降低费用并保留用户自己的理解。调用 DeepSeek 可能产生费用，请以服务商的实际价格与条款为准。

## 隐私与版权

- API Key、AI 对话和本地偏好保存在 `.obsidian/plugins/research-os/data.json`，该文件已被 Git 忽略。
- AI 功能会把必要的论文内容发送到用户配置的 DeepSeek API；背景取色不会上传图片。
...
├── ai-service.js        # DeepSeek、语义分类与论文发现
