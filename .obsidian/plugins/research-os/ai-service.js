...
class AIService {
  ...
  isConfigured() { return Boolean(this.settings.deepseekApiKey.trim()); }
  ...
  async rankPapers(papers) {
    ...
    let parsed;
    try { parsed = JSON.parse(content); } catch { throw new Error("DeepSeek 返回的推荐结果无法解析"); }
  ...
  async generateGuide(noteFile, paperData = null) {
    ...
    const block = `\n\n## AI 导读\n\n> 生成依据：标题与摘要；模型：${this.settings.deepseekModel}；时间：${new Date().toLocaleString()}\n\n${guide}\n`;
    if (/\n## AI 导读\n/.test(current)) {
...
class ResearchAISettingTab extends PluginSettingTab {
  ...
  display() {
    ...
    new Setting(containerEl).setName("DeepSeek API Key").setDesc("在 DeepSeek 开放平台创建").addText(text => {
      text.inputEl.type = "password"; text.setPlaceholder("sk-...").setValue(this.plugin.ai.settings.deepseekApiKey).onChange(async value => { this.plugin.ai.settings.deepseekApiKey = value.trim(); await this.plugin.ai.save(); });
    });
    new Setting(containerEl).setName("模型").addDropdown(dropdown => dropdown
      .addOptions({ "deepseek-v4-flash": "DeepSeek V4 Flash（推荐）", "deepseek-v4-pro": "DeepSeek V4 Pro（深度研究）" })
      .setValue(this.plugin.ai.settings.deepseekModel).onChange(async value => { this.plugin.ai.settings.deepseekModel = value; await this.plugin.ai.save(); }));
    new Setting(containerEl).setName("测试连接").addButton(button => button.setButtonText("测试").onClick(async () => {
      button.setDisabled(true); try { await this.plugin.ai.testConnection(); new Notice("DeepSeek 连接成功"); } catch (e) { new Notice(e.message, 8000); } finally { button.setDisabled(false); }
    }));
